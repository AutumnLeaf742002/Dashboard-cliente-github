<?php
    ...
    if(!empty($_POST))
    {
        ...
        if($rol_logued == "1")
        {
            define("sql", "SELECT citas.Id_agendador, citas.Id as Id, clientes.Primer_nombre as Primer_nombre, clientes.Apellido as Apellido, clientes.Telefono_celular as Telefono_celular, instaladores.Nombre as Nombre, instaladores.Cell as Cell, citas.Fecha as Fecha, citas.Hora as Hora, clientes.Direccion as Direccion, estados_citas.Estado as Estado, roles.Rol as Rol FROM citas JOIN clientes ON clientes.Id = citas.Id_cliente JOIN instaladores ON instaladores.Id = citas.Id_instalador JOIN estados_citas ON estados_citas.Id = citas.Estado_cita JOIN roles ON roles.Id = citas.Rol");
        }
        ...
        {
            define("sql", "SELECT citas.Id_agendador, citas.Id as Id, clientes.Primer_nombre as Primer_nombre, clientes.Apellido as Apellido, clientes.Telefono_celular as Telefono_celular, instaladores.Nombre as Nombre, instaladores.Cell as Cell, citas.Fecha as Fecha, citas.Hora as Hora, clientes.Direccion as Direccion, estados_citas.Estado as Estado, roles.Rol as Rol FROM citas JOIN clientes ON clientes.Id = citas.Id_cliente JOIN instaladores ON instaladores.Id = citas.Id_instalador JOIN estados_citas ON estados_citas.Id = citas.Estado_cita JOIN roles ON roles.Id = citas.Rol WHERE citas.Id_agendador = $id_logued AND citas.Rol = 3");
        }
